...
public class timkiem extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet timkiem</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet timkiem at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
         ...
         try {
                ...
                if (vdv != null) {
                    request.setAttribute("result", vdv);
                } else {
                    request.setAttribute("error", "Không tìm thấy khách hàng.");
                }
            } catch (NumberFormatException e) {
                request.setAttribute("error", "Số tài khoản không hợp lệ!");
            }
